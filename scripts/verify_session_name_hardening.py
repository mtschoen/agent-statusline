"""Verify statusline_lib/render_claude.py's line-1 session-id/session-name
appenders survive a non-string value from the harness payload.
...
from statusline_lib.render_claude import _append_session_id, _append_session_name
...
def _check_append_session_name_non_string(failures):
    result = _strip(_append_session_name("base", 123))
    if result != "base 123":
        failures.append(
...
def _check_append_session_name_none(failures):
    result = _append_session_name("base", None)
    if result != "base":
        failures.append(
...
def _check_append_session_id_non_string(failures):
    result = _strip(_append_session_id("base", 456))
    if result != "base [456]":
        failures.append(
...
def _check_append_session_id_none(failures):
    result = _append_session_id("base", None)
    if result != "base":
        failures.append(
