"""Subagent panel row rendering: payload -> one NDJSON row per task.
...
from .badge import ctx_window_for_model, format_context, format_model_badge
from .base import (
    CTX_DENOM,
    GREEN,
    ORANGE,
...
from .beacon import _find_session_jsonl, format_beacon
from .costfmt import format_cache, format_cost
...
def _format_elapsed(start_time_ms, status, now):
    ...
    if start_time_ms <= 0:
        return ""
    seconds = max(0, int(now * 1000 - start_time_ms) // 1000)
    if seconds < 60:
        return f"{seconds}s"
    if seconds < 3600:
...
def _metrics_for_task(task, parent_transcript_path, session_id, is_lead, now):
    ...
    # One whole transcript per row per render is the receive thread's whole
    # budget several times over, so the walk is served from memory and
    # recomputed on the worker pool. A row with no summary yet renders bare
    # and fills in on the next render.
...
def _row_for_task(task, parent_transcript_path, session_id, now):
    task_id = task.get("id") or ""
    if not task_id:
        return None
    ...
    elapsed = _format_elapsed(task.get("startTime"), task.get("status"), now)
